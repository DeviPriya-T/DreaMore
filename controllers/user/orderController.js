...
// let refundAmount = item.totalProductPrice || (item.price * item.quantity);

// // ✅ Declare outside so it's always accessible
// let remainingAmount = order.orderedItems
//   .filter(i => i._id.toString() !== item._id.toString() && i.status !== 'cancelled')
//   .reduce((acc, i) => acc + (i.totalProductPrice || (i.price * i.quantity)), 0);

// // ✅ Check coupon and reduce refund if condition breaks
...
//       console.log("Coupon condition failed. Removing discount from refund.");
...
// console.log("Cancelled item price:", item.price * item.quantity);
// console.log("Order discount applied:", order.discount);
// console.log("Remaining non-cancelled items total:", remainingAmount);
// console.log("Final refund amount after coupon check:", refundAmount);

// // ✅ Credit the correct amount to wallet
// await creditWallet({
//   userId,
//   amount: refundAmount,
...
// FULLY UPDATED cancelOrder controller with applied logic
...
//         if (coupon && coupon.minimumPrice) {
//   if (remainingAmount < coupon.minimumPrice) {
//     // Revoke full coupon if threshold not met
//     couponRevoked = true;
//     refundAmount -= order.discount;
//     if (refundAmount < 0) refundAmount = 0;
...
//   } else {
//     // Apply proportional discount for cancelled item
//     const totalBefore = order.totalOrderPrice + cancelledAmount;
//     const itemShare = cancelledAmount / totalBefore;
//     const itemDiscount = itemShare * order.discount;
...
const saveFailedOrder = async (req, res) => {
  try {
    ...
    if (!userCart || !selectedAddress) {
      return res.status(400).json({ success: false, message: "Missing cart or address" });
    }
